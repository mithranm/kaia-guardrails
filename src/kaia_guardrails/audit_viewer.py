...
class AuditLogViewer:
    ...
    def __init__(self, log_dir: Path | None = None, project_root: Path | None = None):
        """
    ...
    def _find_log_directory(self, project_root: Path | None = None) -> Path:
        """
    ...
    def load_logs(self, date_filter: str | None = None) -> list[dict[str, Any]]:
        """
    ...
    def analyze_decision_patterns(
        self, entries: list[dict[str, Any]], time_window_hours: int = 24
    ) -> dict[str, Any]:
        ...
        for entry in entries:
            ...
            if event_type == "pre_tool_use":
                ...
                if current_sequence:
                    ...
                    last_tool["files_modified"] = entry.get("metadata", {}).get(
                        "files_modified", []
                    )
                    last_tool["commits_created"] = entry.get("metadata", {}).get(
                    ...
                    if any(
                        ...
                        for keyword in ["blocked", "override", "quality gate", "intervention"]
                    ):
                        hook_interventions.append(
                            {
                                ...
                                "details": result,
                                "session_id": entry.get("session_context", {}).get(
                                    "session_id", "unknown"
                                ),
                            }
                        )
                    ...
                    tool_sequences.append(
                        {
                            ...
                            "ended_at": (
                                current_sequence[-1].get("timestamp")
                                if len(current_sequence) > 1
                                else current_sequence[0].get("timestamp")
                            ),
                        }
                    )
                    current_sequence = []
    ...
    def generate_audit_report(self, analysis: dict[str, Any], verbose: bool = False) -> str:
        ...
        if analysis["hook_interventions"]:
            ...
            if verbose:
                ...
                for intervention in analysis["hook_interventions"][-10:]:  # Last 10
                    report.append(
                        f"  ⚠️  [{intervention['timestamp'][:19]}] {intervention['tool']} - {intervention['intervention_type']}"
                    )
        ...
        for tool, count in sorted(tool_counts.items(), key=lambda x: x[1], reverse=True):
            success_rate = (
                (success_rates[tool]["success"] / success_rates[tool]["total"]) * 100
                if tool in success_rates
                else 100
            )
        ...
        if analysis["focus_interactions"]:
            ...
            if verbose:
                for interaction in analysis["focus_interactions"][-5:]:  # Last 5
                    report.append(
                        f"  📍 [{interaction['timestamp'][:19]}] {interaction['event']} - {interaction['tool']}"
                    )
            report.append("")
        else:
        ...
        if not analysis["hook_interventions"]:
            ...
            report.append(
                "     - Add hooks that can analyze code changes and block dangerous operations"
            )
